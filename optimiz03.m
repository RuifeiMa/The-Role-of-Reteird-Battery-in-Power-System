...
% LD表示标幺值为1时各省各阶段对应的最大负荷------所有典型日天数相同
% for prv =1:province_num
%     for s=1:stage
%         LD(prv,s) = (ydl(prv,s)*100)/(sum(sum(ld(:,:,prv)))*365/12); %Gwh
%     end
% end
...
%         for i=1:T  % 12个典型日
%         C_thermal = [C_thermal,
%                P_c(prv,i,:,s) <= I_onc(prv,i,:,s), %最小发电功率约束(19)
%                P_c(prv,i,:,s) >= 0.4.*I_onc(prv,i,:,s), 
%                P_g(prv,i,:,s) <= I_ong(prv,i,:,s), 
%                P_g(prv,i,:,s) >= 0.3.*I_ong(prv,i,:,s),
%                P_bio(prv,i,:,s) <= I_onbio(prv,i,:,s), 
%                P_bio(prv,i,:,s) >= 0.35.*I_onbio(prv,i,:,s),
%                %有ccs的火电发电约束
%                P_cccs(prv,i,:,s) <= I_oncccs(prv,i,:,s), % 发电功率约束(19)
%                P_cccs(prv,i,:,s) >= 0.4.*I_oncccs(prv,i,:,s),
%                P_gccs(prv,i,:,s) <= I_ongccs(prv,i,:,s), 
%                P_gccs(prv,i,:,s) >= 0.3.*I_ongccs(prv,i,:,s),
%                P_bioccs(prv,i,:,s) <= I_onbioccs(prv,i,:,s), 
%                P_bioccs(prv,i,:,s) >= 0.35.*I_onbioccs(prv,i,:,s),
%  
%                0 <= I_onc(prv,i,:,s) <= I_c(prv,s), %在线容量小于总容量
%                0 <= I_ong(prv,i,:,s) <= I_g(prv,s),
%                0 <= I_onbio(prv,i,:,s) <= I_bio(prv,s),
%                0 <= I_oncccs(prv,i,:,s) <= I_cccs(prv,s,1), % 在线容量约束(20)
%                0 <= I_ongccs(prv,i,:,s) <= I_gccs(prv,s,1),
%                0 <= I_onbioccs(prv,i,:,s) <= I_bioccs(prv,s,1), 
% 
%                % 核电约束
%                0.5.*I_onnu(prv,i,:,s) <= P_nu(prv,i,:,s),
%                P_nu(prv,i,:,s) <= I_onnu(prv,i,:,s),
%                0 <= I_onnu(prv,i,:,s) <= I_nu(prv,s,1),
%                ];
% 
%         end
%     end
...
steppp = 26
% %% Spinning Reserve Requirements during Operation(新)
% num = 0;
% C_Spinning = [];
% 
% for prv= 1:province_num
...
%             for tt = 1:24
%                 num = num+1
%                    C_Spinning=[C_Spinning,
%                        Phot_c(prv,i,tt,s) + Phot_g(prv,i,tt,s) + Phot_csp(prv,i,tt,s) + Phot_bio(prv,i,tt,s) + Phot_cccs(prv,i,tt,s) + Phot_gccs(prv,i,tt,s) + Phot_bioccs(prv,i,tt,s) + Phot_es(prv,i,tt,s) + Phot_ps(prv,i,tt,s) + Phot_h(prv,i,tt,s) + sum(Phot_IF_DC_to(:,prv,i,tt,s)) >= 0.05*LD(prv,s)*ld(tt,i,prv) + 0.05*sum(Phot_IF_DC_to(prv,:,i,tt,s)) + 0.05.*(P_woff(prv,i,tt,s) + P_won(prv,i,tt,s) + P_sc(prv,i,tt,s) + P_sd(prv,i,tt,s)), 
%                        %热备用约束                                                             %% 这里其他类型的发电厂不加吗，下面加了？
%                        0 <= Phot_h(prv,i,tt,s) <= I_h(prv,s,1) - P_h(prv,i,tt,s),                      
%                        0 <= Phot_es(prv,i,tt,s) <= I_es(prv,s)-P_dis(prv,i,tt,s)+P_ch(prv,i,tt,s)
%                        0 <= Phot_es(prv,i,tt,s) <= ES(prv,i,tt,s)*0.9*6,
%                        0 <= Phot_ps(prv,i,tt,s) <= I_ps(prv,s)-P_psdis(prv,i,tt,s)+P_psch(prv,i,tt,s)
%                        0 <= Phot_ps(prv,i,tt,s) <= PS(prv,i,tt,s)*0.75*6,
%                        0 <= Phot_csp(prv,i,tt,s) <= I_csp(prv,s) - P_csp(prv,i,tt,s),
% %                        0 <= Phot_csp(prv,i,tt,s) <= 6*CAP_csp_TES(prv,s)*eff_csp,  
%                        0 <= Phot_csp(prv,i,tt,s) <= CSP_S(prv,i,tt,s)*eff_csp*6,
%                        
%                        ];
% 
%             end
...
%     0 <= Phot_gccs 
%     
%     Phot_bio <= I_onbio-P_bio,
%     Phot_bio <= I_onbio,
%     Phot_bioccs <= I_onbioccs-P_netbioccs
%     Phot_bioccs <= I_onbioccs,
%     
% %     Phot_nu <= I_onnu - P_nu,
%     
%     
%     Phot_c <= I_onc-P_c
%     Phot_c <= 0.2*I_onc,
%     Phot_cccs <= I_oncccs-P_netcccs
%     Phot_cccs <= 0.2*I_oncccs,
%     
%     Phot_g<= I_ong-P_g
%     Phot_g <= 0.7*I_ong,
%     Phot_gccs <= I_ongccs-P_netgccs
%     Phot_gccs <= 0.7*I_ongccs,
%     
%     Phot_IF_DC_to >= 0,
% 
%     
...
%                 num = num+1
%                 C_Spinning = [C_Spinning,
%                     0 <= Phot_IF_DC_to(prv,intr,i,tt,s)<= DC_total_topo_capaity(s,prv,intr) - P_DC_to(prv,intr,i,tt,s),
%                     ];
%             end
%         end
%     end
...
%% 目标函数优化求解
% clear result;
% C = [C_var,C_ERR,C_CAP,C_GAS,C_CF,C_CARBON,C_PowerReserve,C_RegionPowerBalance,C_Lines,C_thermal,C_CCS,C_RENEWS,C_ESS,C_CSP,C_Inertia,C_Spinning];
...
% ops=sdpsettings('solver','gurobi','gurobi.Method',3,'gurobi.InfUnbdInfo',1,'gurobi.Aggregate',0,'gurobi.DualReductions',0);
% ops=sdpsettings('solver','gurobi','gurobi.InfUnbdInfo',1);
ops=sdpsettings('solver','gurobi');
% ops=sdpsettings('solver','gurobi','gurobi.ResultFile','MyFile.lp');
